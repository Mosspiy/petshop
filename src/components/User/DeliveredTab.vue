...
const fetchOrders = async () => {
  try {
    ...
    // กรองเฉพาะออเดอร์ที่มีสถานะ "Shipped"
    orders.value = userOrders.filter(order => order.status === 'Shipped');
    console.log('Shipped orders:', orders.value);
  } catch (err) {
...
// ฟังก์ชันคัดลอกหมายเลขพัสดุ
const copyTrackingNumber = (trackingNumber) => {
  if (!trackingNumber) return;
  
  navigator.clipboard.writeText(trackingNumber)
    .then(() => {
      alert('คัดลอกหมายเลขพัสดุแล้ว!');
    })
    .catch(err => {
...
<template>
  ...
  <div v-else-if="orders.length === 0" class="p-4 max-w-md mx-auto md:max-w-full">
    <div class="card bg-white shadow-md rounded-xl p-8 text-center">
      ...
      <p class="text-gray-600">ยังไม่มีคำสั่งซื้อที่จัดส่งแล้ว</p>
    </div>
  </div>
    ...
    <div v-for="order in orders" :key="order.id" class="card bg-white shadow-md rounded-xl mb-4">
      <div class="card p-4">
        ...
        <div class="pt-2">
          ...
          <p class="text-sm text-right w-full pt-1">จัดส่งสินค้าแล้ว</p>
          
          <!-- เลขติดตามพัสดุ -->
          <p 
            v-if="order.trackingNumber && order.trackingNumber !== '-'" 
            @click="copyTrackingNumber(order.trackingNumber)" 
            class="text-sm text-left pt-5 cursor-pointer"
          >
            <span>หมายเลขพัสดุ {{ order.trackingNumber }}</span>
            <i class="fa-regular fa-copy pl-1"></i>
          </p>
          <p v-else class="text-sm text-left pt-5 text-gray-500">
            ยังไม่มีเลขพัสดุ
          </p>
        </div>
      </div>

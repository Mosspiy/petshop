...
const fetchOrders = async () => {
  try {
    ...
    // กรองเฉพาะออเดอร์ที่มีสถานะ "Pending"
    orders.value = userOrders.filter(order => order.status === 'Pending');
    console.log('Pending orders:', orders.value);
  } catch (err) {
...
<template>
  ...
  <div v-else-if="orders.length === 0" class="p-4 max-w-md mx-auto md:max-w-full">
    <div class="card bg-white shadow-md rounded-xl p-8 text-center">
      <i class="fa-solid fa-clipboard-list text-gray-400 text-4xl mb-2"></i>
      <p class="text-gray-600">ยังไม่มีคำสั่งซื้อที่รอดำเนินการ</p>
    </div>
  </div>
    ...
    <div v-for="order in orders" :key="order.id" class="card bg-white shadow-md rounded-xl mb-4">
      <div class="card p-4">
        ...
        <div class="pt-2">
          ...
          <p class="text-sm text-right w-full pt-1">รอตรวจสอบคำสั่งซื้อ</p>
        </div>
      </div>
    </div>
  </div>
